...
Database ::Database() throw(IOError, MemoryError)
{
    try
    {
        this->vehicletable = new Table<Vehicle>("vechiles.txt");
...
void Database ::fetchAllVechicles() throw(IOError, MemoryError)
{
    ...
    for (string line; getline(this->vehicleTable->fileStream, line);)
    {
        ...
        auto type = VechileType(stoi(components[2]));
        auto seats = stoi(components[3]);
        auto companyName = components[4];
        auto pricePerKm = stod(components[5]);
        auto PUCExpirrationDate = Date(components[6]);

        Storable *record = new Vehicle(registrationNumber, type, seats, companyName, pricePerKm, PUCExpirrationDate, recordId);
        if (!record)
        {
            throw MemoryError();
        }
        this->vehicleTable->record->push_back(record);
    }
...
const vector<const Vehicle *> Database ::getVehicle(Date startDate, Date endDate, VehicleType type) const
{
    ...
    for (auto &vrecord : *this->vehicleTable->records)
    {
        ...
        {
            /* Check if this vehicle is free between start Date and endDate */
            ...
            for (auto &trecord : *this->tripTable->records)
            {
                ...
                if (
                    ...
                    !(trip->getStartDate() <= startDate && trip -
