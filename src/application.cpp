...
void Application::renderAddNewTripMenu() const {
    ...
    for (auto &vehicle: freeVehicles) {
        ...
        cout << "|"
        registerationNo << string(7, ' ') << "|" << seats << string(7 - seats.length(), ' ' << "|" << price << string(
                14 - price.length(), ' ') << "|" << endl;
        int offset = int(freeVehicles.size()) + 2;
        gotoXY(0,12)+offset);
        cout<<registrationNolabel;
        gotoXY(int(registrationNolabel.lenth()),12 + offset);
        getline(cin,registrationNo);
        /*Find vehicle with givern registeration number and create a trip*/
        try{
